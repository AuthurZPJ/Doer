...
- **今日看板**：一屏展示今天所有类别（正在做、未来计划、会议、学习、问题），顶部快速录入
- **Doing**：正在做的任务管理，支持子项（可无限嵌套）、点击文字直接编辑、子项完成追踪
- **未来计划**：待办管理，支持优先级和截止日期，按截止日期分组展示，点击"开始做"自动转入 Doing
- **会议记录**：记录会议标题、内容和日期，支持编辑
- **学习知识点**：记录学习内容，支持展开/折叠
- **当前问题**：记录遇到的问题，支持标记解决
- **周报**：自动汇总本周每日完成项及子项进度，按标签分组，支持导出 Markdown
- **全局搜索**：跨所有模块搜索关键词
